...
app.get('/api/resumo', autenticar, (req, res) => {
  ...
  const totalAssinaturas = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM assinaturas WHERE usuario_id = ? AND ativa = 1`).get(req.usuario.id);
  res.json({
    receitas: totalReceitas.total,
    despesas: totalDespesas.total,
    saldo: totalReceitas.total - totalDespesas.total,
    gastos_por_categoria: gastosCategoria,
    gasto_mensal_assinaturas: totalAssinaturas.total,
  });
});
